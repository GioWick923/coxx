...
class EnvironmentValidator:
    ...
    def _list_ollama_models_from_api(self) -> List[str]:
        host = os.getenv("OLLAMA_HOST", "http://127.0.0.1:11434").rstrip("/")
        req = urlrequest.Request(f"{host}/api/tags", method="GET")
    ...
    def validate_ollama(self) -> CheckResult:
        ...
        if not cli_available:
            ...
            if missing_required:
                ...
                suggestions.append("Instala los modelos faltantes con `ollama pull` o actualiza modelos activos/perfiles.")
            return CheckResult(
        ...
        if proc.returncode == 0:
            models.extend(self._parse_ollama_models(proc.stdout))
        else:
            # si CLI falla, continuar con detección local/API para evitar falsos negativos
            pass
        ...
        if not models and proc.returncode != 0:
            return CheckResult(
                ok=False,
