...
def _parse_ollama_models_from_list_output(raw_output: str) -> List[str]:
    models: List[str] = []
...
def _list_ollama_models_from_api() -> List[str]:
...
def _guess_ollama_models_dir() -> Optional[Path]:
...
def _list_ollama_models_from_fs() -> List[str]:
    models_dir = _guess_ollama_models_dir()
    if not models_dir:
        return []
    ...
    found: List[str] = []
    for file_path in manifests_root.rglob("*"):
        if not file_path.is_file():
            continue
        rel_parts = file_path.relative_to(manifests_root).parts
        # patrón típico: registry.ollama.ai/library/<model>/<tag>
        if len(rel_parts) >= 4 and rel_parts[-3] == "library":
            model_name = rel_parts[-2]
            model_tag = rel_parts[-1]
