...
class OllamaDetectionFallbackTests(unittest.TestCase):
    def test_list_ollama_models_fallback_to_api(self):
        from subprocess import CompletedProcess

        api_payload = {"models": [{"name": "llama3.1:8b"}, {"name": "nomic-embed-text"}]}
        ...
        with patch("rag_mejorado.subprocess.run", return_value=CompletedProcess(args=["ollama", "list"], returncode=1, stdout="", stderr="fail")), patch(
            "rag_mejorado.urlrequest.urlopen", return_value=Resp()
        ):
            models = rag.list_ollama_models()

        self.assertEqual(models, ["llama3.1:8b", "nomic-embed-text"])

    def test_list_ollama_models_fallback_to_fs(self):
        from subprocess import CompletedProcess

        with tempfile.TemporaryDirectory() as td:
            models_root = Path(td) / "models"
            manifest = models_root / "manifests" / "registry.ollama.ai" / "library" / "llama3.1" / "8b"
            manifest.parent.mkdir(parents=True, exist_ok=True)
            manifest.write_text("{}", encoding="utf-8")

            with patch("rag_mejorado.subprocess.run", return_value=CompletedProcess(args=["ollama", "list"], returncode=1, stdout="", stderr="fail")), patch(
                "rag_mejorado._list_ollama_models_from_api", return_value=[]
            ), patch.dict("os.environ", {"OLLAMA_MODELS": str(models_root)}):
                models = rag.list_ollama_models()
...
class BackupSystemTests(unittest.TestCase):
    def test_run_and_list_backups(self):
        with tempfile.TemporaryDirectory() as td:
            tdp = Path(td)
            backup_dir = tdp / "backups"
            history_file = tdp / "history.json"
            benchmark_file = tdp / "benchmark.json"

            history_file.write_text(json.dumps([{"id": "1", "question": "q"}]), encoding="utf-8")
            benchmark_file.write_text(json.dumps([{"run_id": 1, "results": []}]), encoding="utf-8")

            with patch.object(rag, "BACKUP_DIR", backup_dir), patch.object(rag, "HISTORY_FILE", history_file), patch.object(
                rag, "BENCHMARK_RESULTS_FILE", benchmark_file
            ), patch.object(
                rag, "list_model_profiles", return_value={"Rapido": {"chat_model": "llama3.1:8b", "embed_model": "nomic-embed-text"}}
            ), patch.object(
                rag, "get_model_metrics", return_value={"memory": {"rss_mb": 123}}
            ), patch.object(
                rag, "export_configuration",
                return_value={
                    "schema_version": rag.CONFIG_EXPORT_VERSION,
                    "profiles": {},
                    "active": {"chat_model": "llama3.1:8b", "embed_model": "nomic-embed-text"},
                },
            ):
                created = rag.run_backup(reason="manual")
                listed = rag.list_backups(limit=10)

            self.assertTrue(created["ok"])
            self.assertEqual(listed["count"], 1)
            self.assertEqual(listed["items"][0]["backup_id"], created["backup_id"])

    def test_restore_backup_restores_history_and_benchmark(self):
        with tempfile.TemporaryDirectory() as td:
            tdp = Path(td)
            backup_dir = tdp / "backups"
            history_file = tdp / "history.json"
            benchmark_file = tdp / "benchmark.json"
            profiles_file = tdp / "profiles.json"
            backup_dir.mkdir(parents=True, exist_ok=True)

            payload = {
                "schema_version": rag.BACKUP_SCHEMA_VERSION,
                "backup_id": "backup_1",
                "configuration": {
                    "schema_version": rag.CONFIG_EXPORT_VERSION,
                    "profiles": {},
                ...
                "history": [{"id": "h1", "question": "hola"}],
                "metrics": {"benchmark_results": [{"run_id": 9}]},
            }
            (backup_dir / "backup_1.json").write_text(json.dumps(payload), encoding="utf-8")

            with patch.object(rag, "BACKUP_DIR", backup_dir), patch.object(rag, "HISTORY_FILE", history_file), patch.object(
                rag, "BENCHMARK_RESULTS_FILE", benchmark_file
            ), patch.object(
                rag, "MODEL_PROFILES_FILE", profiles_file
            ), patch.object(
                rag, "HISTORY_STORE", rag.QueryHistoryStore(history_file)
            ), patch.object(
                rag, "list_ollama_models", return_value=["llama3.1:8b", "nomic-embed-text"]
            ), patch.object(
                rag, "_save_model_profiles"
            ) as save_profiles:
                out = rag.restore_backup("backup_1", overwrite_profiles=True)

            self.assertTrue(out["ok"])
            self.assertTrue(history_file.exists())
            self.assertTrue(benchmark_file.exists())
            self.assertTrue(save_profiles.called)
