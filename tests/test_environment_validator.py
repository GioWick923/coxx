...
class EnvironmentValidatorTests(unittest.TestCase):
    ...
    def test_validate_ollama_cli_failure_uses_fs_fallback(self):
        validator = EnvironmentValidator(required_models=["llama3.1:8b"])
        ...
        with patch("environment_validator.shutil.which", return_value="/usr/bin/ollama"), patch(
            "environment_validator.subprocess.run", return_value=Proc()
        ), patch.object(validator, "_list_ollama_models_from_fs", return_value=["llama3.1:8b"]), patch.object(
            validator, "_list_ollama_models_from_api", return_value=[]
        ):
            result = validator.validate_ollama()
